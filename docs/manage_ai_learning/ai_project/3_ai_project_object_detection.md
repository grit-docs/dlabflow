...
:::tip 버전 생성 전에 다음 항목을 확인하고 설정할 수 있습니다.
- 학습 파일 정보
- 학습·검증·시험 데이터 분할
- 데이터 전처리
- 학습 데이터 증강
...
데이터 전처리는 학습 전에 이미지의 형식이나 특성을 조정하는 과정입니다.

![](./img/ai_project_049.png)
...
![인공지능 학습 이미지](./img/ai_project_041.png)
...
각 모델 버전은 Small, Medium, Large, XLarge 등 다양한 크기로 제공될 수 있습니다. 모델 크기가 클수록 더 많은 연산 자원과 학습 시간이 필요할 수 있습니다.
:::


### 하이퍼파라미터 고급 설정

학습 이미지 크기, 최적화 알고리즘 등 기본 제공되는 설정을 사용자가 커스터마이징하고자 할 때 사용하는 기능입니다.
...
| 기법 | 설명 |
|:---:|---|
| **다중 스케일** | 학습 과정에서 이미지의 크기를 무작위로 변환하여 다양한 크기의 객체를 인식하기 위해 사용합니다. |
| **코사인 학습률 스케줄러** | 학습 과정에서 학습률을 코사인 곡선 형태로 점진적으로 감소시켜 최적화 과정을 원활하게 하기 위해 사용합니다. |
| **자동 혼합 정밀도** | 메모리 사용량을 줄이고 모델 성능에 미치는 영향을 최소화하면서 학습 속도를 높이기 위해 사용합니다. |
| **기울기 초기 속도 변경** | 기울기 업데이트 속도를 천천히 올리기 위해 사용합니다. |
| **학습 초기 속도 변경** | 학습 속도를 천천히 올리기 위해 사용합니다. |
...
| 항목 | 설명 |  설정 유형   |
|:---:|---|:------------:|
| **초기 학습률** | 학습 속도를 천천히 올리기 위해 사용합니다. | 0.001 ~ 0.05 |
| **최종 학습률** | 학습 종료 시 가중치 업데이트 속도를 초기 학습률 대비 어느 정도로 줄일지 결정하기 위해 사용합니다. |  0.01 ~ 0.1  |
| **학습률 초기 증가율** | 학습률을 낮은 값에서 점진적으로 증가시키는데 사용합니다. |  0.1 ~ 0.2   |
| **학습 레이어 고정** | 학습 가능한 파라미터 수를 줄이기 위해 사용합니다. |    5 ~ 20    |
| **학습 조기 종료** | 모델 성능이 정체될 때 학습을 중단하기 위해 사용합니다. |  0.1 ~ 0.3   |
...
| 구분 | 설명 |    설정 유형    |
|:---:|---|:---------------:|
| **Close Mosaic** | 학습 후반부에 Mosaic 증강을 종료할 시점을 설정합니다. |    0.1 ~ 0.3    |
| **기울기 반영률** | 학습 과정에서 계산된 기울기를 가중치에 반영하는 정도를 설정합니다. |   0.85 ~ 0.99   |
| **과적합 방지율** | 학습 데이터에 과도하게 맞춰지는 것을 줄이기 위한 비율을 설정합니다. |     0 ~ 0.5     |
| **가중치 감소율** | 가중치가 지나치게 커지는 것을 방지하기 위해 감소를 적용하는 비율입니다. | 0.00001 ~ 0.001 |
...
| 구분 | 설명 | 설정 유형 |
|:---:|---|:---------:|
| **객체 위치 탐지 오차 허용률** | Bounding Box 좌표를 얼마나 정확하게 맞출지 결정하는데 사용합니다. |  3 ~ 15   |
| **클래스 분류 오차 허용률** | 클래스를 얼마나 정확하게 맞출지 결정하는데 사용합니다. |  0.1 ~ 3  |
...
|       ![](./img/ai_project_017.png)      |                   ![](./img/ai_project_018.png)                                              |
...
|:---:|-------------------------------|-------------------------------|
| PyTorch에서 학습한 모델을 저장하고 PyTorch 환경에서 학습을 이어가거나 추론할 때 사용하는 형식입니다.| 모델 구조와 가중치를 표준 형식으로 저장하여, PyTorch를 비롯한 다양한 딥러닝 프레임워크와 추론 엔진에서 사용할 수 있는 교환 형식입니다. | OpenVINO 추론 엔진에 맞게 변환한 형식으로, Intel CPU·GPU·VPU 등 지원되는 하드웨어에서 효율적으로 추론할 수 있습니다.  |
| ![](./img/ai_project_019.png) | ![](./img/ai_project_020.png) | ![](./img/ai_project_021.png) |
