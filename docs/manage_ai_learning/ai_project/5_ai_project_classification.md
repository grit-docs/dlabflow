...
![](./img/ai_project_030.png)

### 최적화 알고리즘

하이퍼파라미터를 탐색하는 방법을 선택합니다.
...
| 항목 | 설명 | 설정 유형 |
|:---:|---|:---:|
| **최적화 탐색 횟수** | 평가할 하이퍼파라미터 조합의 수입니다. 값이 클수록 더 많은 조합을 확인하지만 탐색 시간이 늘어납니다. | 수치 |
| **학습 조기 종료** | 성능이 일정 횟수 동안 개선되지 않을 때 학습을 중단하는 기준입니다. 불필요한 학습 시간과 과적합을 줄이는 데 도움이 됩니다. | 수치 |
| **최소 학습률** | 탐색할 학습률의 최솟값입니다. | 수치 |
| **최대 학습률** | 탐색할 학습률의 최댓값입니다. | 수치 |

![](./img/ai_project_031.png)
...
![](./img/ai_project_037.png)

| 실행 형식 | 설명 |
|:---:|---|
| **Scikit-learn** | Scikit-learn 호환 모델을 Python에서 불러와 추론할 수 있는 `.pkl` 형식으로 다운로드합니다. |
| **ONNX** | 모델을 표준 교환 형식으로 변환하여 ONNX Runtime 등 다양한 추론 환경에서 사용할 수 있습니다. |

![](./img/ai_project_038.png)

ONNX로 다운로드할 때 `Dynamic`을 활성화하면 여러 샘플을 한 번에 처리하는 입력 형태를 동적으로 사용할 수 있습니다. `연산자 집합`은 대상 추론 환경이 지원하는 ONNX 연산자 집합 버전에 맞게 선택해야 합니다.
