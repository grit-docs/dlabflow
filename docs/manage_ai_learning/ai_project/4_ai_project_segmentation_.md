...
버전 생성은 선택한 데이터셋과 학습 설정을 바탕으로 모델 학습을 준비하는 과정입니다. 버전을 생성하면 학습에 사용할 데이터와 설정이 해당 버전에 저장되고, 설정한 전처리와 데이터 증강이 학습에 적용됩니다.
...
| 구분 | 내용 |
|:---:|---|
| **파일 수** | 선택한 데이터셋에 포함된 이미지 파일 수입니다. 실제 학습에 사용할 수 있는 파일 수와는 다를 수 있습니다. |
| **라벨링 수** | 선택한 데이터셋에 포함된 객체 라벨링의 수입니다. |
| **클래스 수** | 선택한 데이터셋에 포함된 객체 클래스의 수입니다. |

학습 파일 정보는 데이터셋의 규모와 라벨링 상태를 확인하기 위한 참고 정보입니다. 실제 학습에 사용되는 이미지 수는 이미지 형식, 라벨링 상태 또는 전처리 결과에 따라 달라질 수 있습니다.
...
| **검증 데이터** | 학습 중 모델의 성능을 확인하고 모델 설정을 조정하는 데 사용하는 데이터입니다. |
...
데이터 전처리는 학습 전에 이미지의 형식이나 특성을 조정하는 과정입니다. 전처리를 통해 입력 형식을 맞추고 모델이 학습하기 좋은 형태로 데이터를 준비할 수 있습니다.
...
이미지와 라벨 마스크에 동일한 공간 변환이 적용되어야 하므로, 데이터 증강은 실제 객체의 방향과 촬영 환경을 고려하여 선택해야 합니다. 실제 환경과 관계없는 변형을 과도하게 적용하면 분할 성능이 떨어질 수 있습니다.
...
현재 인스턴스 분할 프로젝트에서는 고급 하이퍼파라미터 설정을 제공하지 않습니다. 모델 선택, 배치 크기, 학습 횟수만 설정할 수 있습니다.
...
IoU 값은 0에서 1 사이이며, 1에 가까울수록 예측 영역과 실제 영역이 정확하게 일치한다는 의미입니다. 예측 위치가 크게 다르거나 마스크 형태가 실제 객체와 다르면 IoU가 낮아집니다.

### 모델 평가

학습이 완료된 모델은 모델 평가 기능을 통해 새로운 이미지에 적용하고 추론 결과를 확인할 수 있습니다. 평가 결과에서는 다음 정보를 확인할 수 있습니다.
...
실제 사용 환경이 충분히 반영된 이미지를 사용해야 평가 결과를 실제 성능에 가깝게 해석할 수 있습니다.
