...
사용자가 AI 학습을 위하여 업로드한 데이터 중 실제로 학습하고자 하는 **데이터 선택 과정**으로, 사용자가 업로드한 데이터를 바탕으로 다양한 학습 데이터셋을 생성할 수 있습니다.
...
![dataset_create](./img/dataset_delete.png)


## 학습 데이터 선택

사용자가 기존에 업로드한 저장소들이 표시되고 이 중 학습하고자 하는 데이터셋이 있으면, `활성화` 버튼을 통해 데이터를 활성화할 수 있습니다.

삭제 절차는 데이터셋 이름을 확인하는 것으로 진행 됩니다.

![dataset_create](./img/dataset_select.png)


## 라벨링 업로드

소유한 저장소 이미지 파일에 대한 라벨링 데이터를 시스템에 업로드할 수 있습니다.
...
`현재는 PascalVOC, YOLO, COCO 형태의 라벨링 업로드 기능만을 제공합니다.`
